...
echo "SAVING MOTIFNET RESUlT"
#python models/eval_stanford_motif.py -m sgcls -model motifnet -order leftright -nl_obj 2 -nl_edge 4 -b 6 -clip 5 \
#    -p 100 -hidden_dim 512 -pooling_dim 4096 -lr 1e-3 -ngpu 1 -test -ckpt checkpoints/motifnet/vgrel-motifnet-sgcls.tar -nepoch 50 -use_bias -cache motifnet_sgcls

python models/save_base_result.py -m predcls -model_s_m motifnet -b 20 -clip 5 -p 20 \
    -p 100 -pooling_dim 4096 -lr 1e-3 -ngpu 1 -test -ckpt checkpoints/motifnet/vgrel-motifnet-sgcls.tar -nepoch 50 | tee  ./logs/save_motif_predcls.txt
