...
parser.add_argument(
    ...
    type=str,
    default='visualization/motif_predcls_mask_ori_stat'
    # default='visualization/saves'
)

parser.add_argument(
    '-cache_dir_predcls',
    dest='cache_dir_predcls',
    help='dir to load cache predicted results',
    type=str,
    default='cache/motifnet_predcls'
)

parser.add_argument(
    '-cache_dir_predcls_mask',
    dest='cache_dir_predcls_mask',
    help='dir to load cache predicted results',
    type=str,
    default='cache/motifnet_glat_predcls_mask'
)
...
def visualize_pred_gt(pred_entry_ori, pred_entry_mask, gt_entry, ind_to_classes, ind_to_predicates, image_dir, graph_dir, top_k=50, save_format='pdf'):
    ...
    assert (pred_entry_ori['pred_classes'] == pred_entry_mask['pred_classes']).all()
    pred_classes = pred_entry_ori['pred_classes']
    ...
    pred_rel_inds_ori = pred_entry_ori['pred_rel_inds']
    pred_rel_inds_mask = pred_entry_mask['pred_rel_inds']

    rel_scores_ori = pred_entry_ori['rel_scores']
    rel_scores_mask = pred_entry_mask['rel_scores']
    ...
    pred_rels_ori = np.column_stack((pred_rel_inds_ori, 1+rel_scores_ori[:,1:].argmax(1)))
    pred_rels_mask = np.column_stack((pred_rel_inds_mask, 1+rel_scores_mask[:,1:].argmax(1)))
    pred_rels_ori = pred_rels_ori[:top_k]
    pred_rels_mask = pred_rels_mask[:top_k]
        ...
        for rel in rels:
            if pred_rel_ori[0] == rel[0] and pred_rel_ori[1] == rel[1]:
                if pred_rel_ori[2] == rel[2]:
                    if pred_rels_mask[idx, 2] == rel[2]:
                        u.edge(str(rel[0]), str(rel[1]), label=ind_to_predicates[rel[2]], color='forestgreen')
                    else:
                        u.edge(str(rel[0]), str(rel[1]), label='->'+ind_to_predicates[pred_rels_mask[idx, 2]]+'('+ind_to_predicates[rel[2]]+')', color='red')

                else:
                    if pred_rels_mask[idx, 2] == rel[2]:
                        label_str = ind_to_predicates[pred_rel_ori[2]] + '->mask->' + ind_to_predicates[pred_rels_mask[idx, 2]]
                        u.edge(str(rel[0]), str(rel[1]), label=label_str, color='blue')
                        corrective_number += 1
                    else:
                        label_str = ind_to_predicates[pred_rel_ori[2]] + '->mask->' + ind_to_predicates[pred_rels_mask[idx, 2]] + '('+ ind_to_predicates[rel[2]] +')'
                        u.edge(str(rel[0]), str(rel[1]), label=label_str, color='red')
    ...
    u.render(filename=os.path.join(graph_dir, str(corrective_number)+'_'+fn.split('/')[-1].split('.')[-2]),view=False, cleanup=True)


def visualize_pred_gt_ori(pred_entry, gt_entry, ind_to_classes, ind_to_predicates, image_dir, graph_dir, top_k=50, save_format='pdf'):
    ...
    pred_rels = np.column_stack((pred_rel_inds, 1+rel_scores[:,1:].argmax(1)))
    top_k = int(0.3 * pred_rels.shape[0]) if pred_rels.shape[0]<100 else 30
    pred_rels = pred_rels[:top_k]
        ...
        for rel in rels:
            if pred_rel[0] == rel[0] and pred_rel[1] == rel[1]:
                ...
                if pred_rel[2] == rel[2]:
                    ...
                    u.edge(str(rel[0]), str(rel[1]), label=label_str, color='red')
        if flag_bg:
            if pred_rel[0] not in rels:
                u.node(str(pred_rel[0]), label=ind_to_classes[pred_classes[pred_rel[0]]], color='black')
            if pred_rel[1] not in rels:
                u.node(str(pred_rel[1]), label=ind_to_classes[pred_classes[pred_rel[1]]], color='black')
            u.edge(str(pred_rel[0]), str(pred_rel[1]), label=ind_to_predicates[pred_rel[2]], color='yellow')

    for rel in rels:
        flag_rel_has_pred = False
        for pred_rel_ori in pred_rels:
            if pred_rel_ori[0] == rel[0] and pred_rel_ori[1] == rel[1]:
                flag_rel_has_pred = True
                break
        if not flag_rel_has_pred:
    ...
    u.render(filename=os.path.join(graph_dir, fn.split('/')[-1].split('.')[-2]),view=False, cleanup=True)




with open(args.cache_dir_predcls, 'rb') as f:
    pred_entries_predcls = pkl.load(f)

# with open(args.cache_dir_predcls_mask, 'rb') as f:
#     pred_entries_predcls_mask = pkl.load(f)

# for i, (pred_entry_ori, pred_entry_mask) in enumerate(tqdm(zip(pred_entries_predcls, pred_entries_predcls_mask))):
#     # if i == 500:
#     #     break
#     gt_entry = {
...
#
#     visualize_pred_gt(pred_entry_ori, pred_entry_mask, gt_entry, ind_to_classes, ind_to_predicates, image_dir=image_dir, graph_dir=graph_dir, top_k=50)
