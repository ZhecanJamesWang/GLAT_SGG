...
#     type=str,
#     default='visualization/motif_sgcls_baseline_fusion'
# )
...
parser.add_argument(
    ...
    help='dir to save visualization files',
    type=str,
    default='visualization/motif_sgcls_baseline_mask'
)
...
#     help='dir to load cache sgcls results',
#     type=str,
#     default='/home/tangtangwzc/KERN/cache/motif_predcls_2020_0216_0959'
# )
...
parser.add_argument(
    ...
    help='dir to load cache sgcls results',
    type=str,
    default='cache/motifnet_glat_sgcls_mask_logit_thres_fintuned_v3_0.35'
)
...
def visualize_pred_gt(pred_entry_baseline, pred_entry_change, gt_entry, ind_to_classes, ind_to_predicates, image_dir, graph_dir, top_k=50, save_format='pdf'):
    ...
    rel_scores_ori = pred_entry_baseline['rel_scores']
    rel_scores_change = pred_entry_change['rel_scores']
    ...
    pred_rels_change = np.column_stack((pred_rel_inds_change, 1+rel_scores_change[:,1:].argmax(1)))
    ...
    u.render(filename=os.path.join(graph_dir, str(corrective_pred_number+corrective_entity_number)+'_'+fn.split('/')[-1].split('.')[-2]),view=False, cleanup=True)

    # print("correct_entity_num_total")
...
for i, (pred_entry_baseline, pred_entry_change) in enumerate(tqdm(zip(pred_entries_baseline, pred_entries_change))):
    if i == 500:
        break
    gt_entry = {
    ...
    visualize_pred_gt(pred_entry_baseline, pred_entry_change, gt_entry, ind_to_classes, ind_to_predicates, image_dir=image_dir, graph_dir=graph_dir, top_k=50)
