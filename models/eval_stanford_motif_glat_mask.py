...
def val_batch(batch_num, b, evaluator,evaluator_multiple_preds, evaluator_list, evaluator_multiple_preds_list, accs):
    ...
    for i, det in enumerate(det_res):
        ...
        # only take nobg samples
        # pdb.set_trace()
        rels_i_b100_nobg = []
        pred_scores_i_b100_nobg = []
        for i in range(rels_i_b100.shape[0]):
            if (rels_i_b100[i][0], rels_i_b100[i][1]) in dict_gt:
                rels_i_b100_nobg.append(np.expand_dims(rels_i_b100[i], axis=0))
                pred_scores_i_b100_nobg.append(np.expand_dims(pred_scores_i_b100[i], axis=0))
        if len(rels_i_b100_nobg) != 0:
            rels_i_b100 = np.concatenate(rels_i_b100_nobg, axis=0)
            pred_scores_i_b100 = np.concatenate(pred_scores_i_b100_nobg, axis=0)
            print('no gt pair in top 100')
        else:
            rels_i_b100 = rels_i_b100
            pred_scores_i_b100 = pred_scores_i_b100
        pred_entry['pred_rel_inds'] = rels_i_b100
        pred_entry['rel_scores'] = pred_scores_i_b100
