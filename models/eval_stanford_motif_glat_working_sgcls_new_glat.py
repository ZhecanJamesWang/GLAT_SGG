...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    pred_label_predicate, base_predicate_one_hot, output_predicate_one_hot = soft_merge3(node_logit_dists, pred_label_predicate_logit_glat, node_type, 0, conf.temp_model)
    ent_dists, pred_label_entities = rearrange_useless(ent_dists, pred_label_entities_glat, useless_entity_id, node_type)
    pred_label_entities, base_entities_one_hot, output_entities_one_hot = soft_merge3(ent_dists, pred_label_entities_logit_glat, node_type, 1, conf.temp_model)
    ...
    return pred_label_predicate, pred_label_entities, comparison_one_hot, change_list, pred_label_predicate_logit_glat, pred_label_entities_logit_glat, node_logit_dists, ent_dists
...
def glat_postprocess(pred_entry, if_predicting=False):
    ...
    if conf.glat_on:
        ...
        pred_label_predicate, pred_label_entities, comparison_one_hot, change_list, pred_label_predicate_logit_glat, pred_label_entities_logit_glat,  pred_label_predicate_logit_base, pred_label_entities_logit_base = glat_wrapper(total_data, useless_entity_id)
        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
