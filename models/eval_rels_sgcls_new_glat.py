...
if conf.glat_on:
    ...
    # # # new finetune sgcls kern new glat weight
    # path = '/home/haoxuan/code/KERN/checkpoints/train_glat_kern_sgcls_v3/kern-24.tar'
    # path = ('/home/haoxuan/code/KERN/checkpoints/train_glat_kern_sgcls_v31/kern-12.tar')

    # # # new finetune predcls kern new glat weight
    # path = ('/home/haoxuan/code/KERN/checkpoints/train_glat_kern_predcls_v31/kern-16.tar')

    # # # new finetune sgdet kern new glat weight
    # path = 'checkpoints/train_glat_kern_sgdet_v31/kern-14.tar'

    # # # new finetune sgcls debiased kern new glat weight
    # path = ('/home/haoxuan/code/KERN/checkpoints/train_glat_kern_debiasedv1_sgcls_v3/kern-23.tar')
    path = ('checkpoints/train_glat_kern_debiasedv1_sgcls_v31/kern-13.tar')

    # # # new finetune sgdet debiased kern 1 new glat weight
    # path = ("/home/haoxuan/code/KERN/checkpoints/train_glat_kern_debiasedv1_sgdet_epoch1_v31/kern-10.tar")

    # # # # new finetune predcls debiased kern new glat weight
    # path = ('/home/haoxuan/code/KERN/checkpoints/train_glat_kern_debiasedv1_predcls_v31/kern-17.tar')
...
def soft_merge3(logit_base, logit_glat, node_type, type, temp_model):
    ...
    if type == 0:
        ...
        logit_base_predicate_raw = logit_base.squeeze(0)[index].view(-1, 52)
    else:
        ...
        logit_base_predicate_raw = logit_base

    logit_base_predicate = softmax_1(logit_base_predicate_raw).data
    ...
    return output_logit_predicate, logit_base_predicate_one_hot, output_predicate_one_hot, logit_base_predicate_raw
...
def glat_wrapper(total_data, useless_entity_id, mode):
    ...
    pred_label_predicate_glat = pred_label[0]  # flatten predicate (B*N, 51)
    pred_label_entities_glat = pred_label[1]  # flatten entities

    pred_label_predicate_logit_glat = pred_label[2]
    pred_label_entities_logit_glat = pred_label[3]

    # vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
    logit_glat_predicate_one_hot = torch.max(pred_label_predicate_glat[:,1:-1], dim=1)[1]
    index = (node_type == 0).squeeze(0).unsqueeze(-1).repeat(1, 52)
    logit_base_predicate = node_logit_dists.squeeze(0)[index].view(-1, 52)
    logit_base_predicate_one_hot = torch.max(logit_base_predicate[:, 1:-1], dim=1)[1]
    # ========================

    logit_glat_entities = pred_label_entities_glat[:, :-2]
    ...
    pred_label_predicate, base_predicate_one_hot, output_predicate_one_hot, logit_base_predicate_raw = soft_merge3(node_logit_dists, pred_label_predicate_logit_glat, node_type, 0, conf.temp_model)
    ent_dists, pred_label_entities = rearrange_useless(ent_dists, pred_label_entities_glat, useless_entity_id, node_type)
    pred_label_entities, base_entities_one_hot, output_entities_one_hot, logit_base_entities_raw = soft_merge3(ent_dists, pred_label_entities_logit_glat, node_type, 1, conf.temp_model)
    ...
    return pred_label_predicate, pred_label_entities, comparison_one_hot, change_list, pred_label_predicate_logit_glat, pred_label_entities_logit_glat, logit_base_predicate_raw, logit_base_entities_raw
...
def glat_postprocess(pred_entry, if_predicting=False):
    ...
    if conf.glat_on:
        ...
        pred_label_predicate, pred_label_entities, comparison_one_hot, change_list, pred_label_predicate_logit_glat, pred_label_entities_logit_glat,  pred_label_predicate_logit_base, pred_label_entities_logit_base = glat_wrapper(total_data, useless_entity_id, conf.mode)
        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    if conf.glat_on:
        ...
        extra_entry['pred_label_predicate_logit_glat'] = pred_label_predicate_logit_glat
        extra_entry['pred_label_entities_logit_glat'] = pred_label_entities_logit_glat
