...
def get_optim(lr, last_epoch=-1):
    ...
    if conf.adam:
        ...
        # v2opt
        optimizer = optim.Adam(params, weight_decay=5e-4, lr=lr)
    else:
...
def train_batch(b, train_results, train_bias_logits, train_det_ress, epoch_num, batch_num, accs, verbose=False):
    ...
    pred_label_alltop = pred_entry['rel_scores'][:, 1:].max(1)[1] + 1
    gt_label_alltop = result.rel_labels[:, -1][b_100_idx]
    ...
    for i in range(len(num_predicates)):
        ...
        keep_num = math.ceil(num_predicates[i]*0.2) if num_predicates[i] <= 100 else 20
    ...
    for i in range(len(num_predicates)):
        ...
        nobg_idx_initop = torch.nonzero(gt_label_alltop[idx_predicates_inalltop_i])
        if len(nobg_idx_initop) != 0:
            nobg_idx_initop = nobg_idx_initop.squeeze(-1).data
            mask_idx_innobg = torch.nonzero(pred_label_alltop[idx_predicates_inalltop_i][nobg_idx_initop]
                                            != gt_label_alltop[idx_predicates_inalltop_i][nobg_idx_initop])
            mask_idx_initop = nobg_idx_initop[mask_idx_innobg.squeeze(-1).data] if len(mask_idx_innobg) !=0 else None
        else:
            mask_idx_initop = None

        if mask_idx_initop is not None:
            mask_idx_initop = mask_idx_initop
    ...
    nobg_idx_ininput = torch.nonzero(gt_label_alltop[input_pred_idxs_inalltop])[:, 0].data

    mask_idxs_innobg = torch.nonzero(pred_label_alltop[input_pred_idxs_inalltop][nobg_idx_ininput]
                                      != gt_label_alltop[input_pred_idxs_inalltop][nobg_idx_ininput])[:, 0].data

    mask_idxs_ininput = nobg_idx_ininput[mask_idxs_innobg]
...
def val_batch(batch_num, b, evaluator, evaluator_multiple_preds, evaluator_list,
    ...
    for i, det in enumerate(det_res):
        ...
        keep_num = math.ceil(num_predicate*0.2) if num_predicate <= 100 else 20
        ...
        for i in range(len(rels_i_b100)):
            if (int(rels_i_b100[i][0]), int(rels_i_b100[i][1])) in dict_gt:
                ...
                if int(pred_lbl) not in dict_gt[(int(rels_i_b100[i][0]), int(rels_i_b100[i][1]))]:
                    mask_idxs_intop100.append(i)

        if len(mask_idxs_intop100) == 0:
            mask_in_keep_num = 0
        else:
            mask_in_keep_num = np.where(np.array(mask_idxs_intop100) < keep_num)[0].shape[0]
