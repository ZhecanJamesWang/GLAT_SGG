...
if conf.use_obj_knowledge:
    print('Use obj knowledge')
else:
    print('Not Use obj knowledge')

if conf.use_rel_knowledge:
    print('Use rel knowledge')
else:
    print('Not Use rel knowledge')

if len(conf.reweight_matrix_ent) > 1:
    print("Use entity reweight")
else:
    print("Not use entity reweight")
...
detector = KERN(classes=train.ind_to_classes, rel_classes=train.ind_to_predicates,
                ...
                return_vis_fea=False, return_top100=False, return_unbias_logit=False)
...
def get_class_weights(num_class, counts_path, class_volume=1000):
    with open(counts_path, 'rb') as fin:
        class_counts = pickle.load(fin)
    beta = (class_volume - 1.0) / class_volume
    class_weights = (1.0 - beta) / (1 - (beta ** class_counts))
    if class_counts[0] == 0:
        class_weights[0] = 0
    class_weights *= float(num_class) / np.sum(class_weights)
...
def train_batch(b, verbose=False):
    ...
    losses = {}
    # Bal weight >>>>>>>
    rel_class_weights = get_class_weights(num_class=len(ind_to_predicates), counts_path=conf.reweight_matrix_pred)
    ...
    if conf.use_ggnn_obj: # if not use ggnn obj, we just use scores of faster rcnn as their scores, there is no need to train
        if len(conf.reweight_matrix_ent) > 1:
            # Bal weight >>>>>>>
            ent_class_weights = get_class_weights(num_class=len(ind_to_classes), counts_path=conf.reweight_matrix_ent,
                                                  class_volume=3000)
            ent_class_weights = Variable(torch.from_numpy(ent_class_weights)).float().cuda()
            losses['class_loss'] = F.cross_entropy(result.rm_obj_dists, result.rm_obj_labels, weight=ent_class_weights)
            # <<<<<<<<<<
        else:
