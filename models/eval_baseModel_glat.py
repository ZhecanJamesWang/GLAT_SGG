...
def val_batch(batch_num, b, evaluator,evaluator_multiple_preds, evaluator_list, evaluator_multiple_preds_list):
    ...
    # with np.load('stanford_val_output.npy') as det_res:
...
if conf.cache is not None and os.path.exists(conf.cache):
    ...
    dir = "/home/tangtangwzc/KERN/intermediate_100/"
    ...
    for file in tqdm(files):
        try:
            det_res = np.load(os.path.join(dir, file), allow_pickle=True)
            ...
            for i, det in enumerate(det_res):
                ...
                #           evaluator_multiple_preds_list)
                val_batch(conf.num_gpus * i, det, evaluator, evaluator_multiple_preds, evaluator_list,
                          evaluator_multiple_preds_list)
