...
def train_batch(b, train_results, train_bias_logits, train_det_ress, epoch_num, batch_num, accs, verbose=False):
    ...
    for i in range(len(num_predicates)):
        ...
        gt_label = result.rel_labels[b_100_idx[idx_predicates_inalltop[i]]][:, -1].data
        pred_label = pred_entry['rel_scores'][idx_predicates_inalltop[i]][:, 1:].max(1)[1].data + 1
        fg_idx_initop = torch.nonzero(gt_label)
        if len(fg_idx_initop) != 0:
            fg_idx_initop = fg_idx_initop[:, 0]
            wrong_pred_idx_infg = torch.nonzero(gt_label[fg_idx_initop] != pred_label[fg_idx_initop])
        else:
            wrong_pred_idx_infg = []

        if len(wrong_pred_idx_infg) != 0:
            wrong_pred_idx_infg = wrong_pred_idx_infg[:, 0]
            wrong_pred_idx_initop = fg_idx_initop[wrong_pred_idx_infg]
            wrong_pred_idx_inalltop = wrong_pred_idx_initop + sum(num_predicates[:i]) if i > 0 else wrong_pred_idx_initop
            wrong_pred_idx_inalltop = wrong_pred_idx_inalltop.tolist()
        else:
            wrong_pred_idx_inalltop = []
            wrong_pred_idx_initop = []
        ...
        if len(mask_idx_initop) != 0:
            ...
            for wrong_idx in wrong_pred_idx_inalltop:
                if wrong_idx not in mask_idx_inalltop:
                    for loc in range(len(mask_idx_inalltop)):
                        if loc == 0 and wrong_idx < mask_idx_inalltop[loc]:
                            insert_loc = loc
                            break
                        elif wrong_idx > mask_idx_inalltop[loc-1] and wrong_idx<mask_idx_inalltop[loc]:
                            insert_loc = loc
                            break
                        elif loc == len(mask_idx_inalltop)-1 and wrong_idx > mask_idx_inalltop[loc]:
                            insert_loc = loc + 1
                            break
                    mask_idx_inalltop.insert(insert_loc, wrong_idx)
            mask_idxs_inalltop += mask_idx_inalltop
            # mask_in_keep_num = len(torch.nonzero(mask_idx_initop < keep_nums[i]))
            mask_in_keep_num = len(torch.nonzero(torch.Tensor(mask_idx_inalltop).long() < (keep_nums[i] + sum(num_predicates[:i]))))
            input_pred_num = len(mask_idx_inalltop) + keep_nums[i] - mask_in_keep_num
        else:
            mask_idx_inalltop = []
            mask_in_keep_num = 0
            input_pred_num = keep_nums[i]

        input_pred_nums.append(input_pred_num)

        for pred_id in range(input_pred_num):
            if pred_id < keep_nums[i]:
                input_pred_idxs_inalltop.append(keep_idx_inalltop[pred_id])
                if pred_id + sum(num_predicates[:i]) in mask_idx_inalltop:
                    mask_idx_ininput = pred_id + sum(input_pred_nums[:i]) if i>0 else pred_id
                    mask_idxs_ininput.append(mask_idx_ininput)
            else:
    ...
    assert len(mask_idxs_inalltop) == len(mask_idxs_ininput)
    ...
    # pdb.set_trace()
    fg_idx_inmask = torch.nonzero(result.rel_labels[b_100_idx[mask_idxs_inalltop]][:, -1])[:,0]

    losses['rel_loss'] = F.cross_entropy(pred_scores_sorted_b_100[mask_idxs_ininput][fg_idx_inmask], result.rel_labels[b_100_idx[mask_idxs_inalltop]][fg_idx_inmask][:, -1])
